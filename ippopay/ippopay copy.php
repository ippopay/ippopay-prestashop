...
class IppoPay extends PaymentModule
{
    ...
    private function _displayForm()
    {
        ...
        <p><form action='{$_SERVER['REQUEST_URI']}' method='post'>
                <fieldset>
                <legend><img src='../img/admin/access.png' />{$modippopay}</legend>
                        <table border='0' width='500' cellpadding='0' cellspacing='0' id='form'>
                                <tr>
                                        <td colspan='2'>
                                                {$modippopayDesc}<br /><br />
                                        </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                        <td width='130' title='{$modredirectDesc}' >{$modredirectDesc}</td>
                                        <td>
                                                <input type='checkbox' name='ENABLE_REDIRECT_OPTION' style='width: 300px;' {$modredirect} />
                                        </td>
                                </tr>
                                <tr>
    ...
    private function _displayippopay()
    {
        ...
        $modconfirm = $this->l('');
        $this->html .= '.ippopay-payment-section *{margin:0;padding:0;box-sizing:border-box}.ippopay-payment-section img{max-width:100%}.ippopay-payment-section{font-family:inherit;float:left;width:100%}.ippopay-payment{float:left;width:100%}.ippopay-title{float:left;width:100%;font-size:25px;color:#585757;margin-bottom:15px}.ippopay-desc{float:left;width:100%}.ippopay-desc img{float:left}.ippopay-cont{float:left;width:calc (100% - 200px);margin-left:20px}.bld-cont{float:left;width:100%;color:#3a3838}.sub-desc1{float:left;width:100%;color:#4d4c4c}.ippopay-config{float:left;width:100%;padding-top:80px}.config-title{float:left;width:100%;color:#1b1b1b;border-bottom:1px solid grey;padding-bottom:5px;margin-bottom:20px}.sub-desc2{float:left;width:100%;color:#727272;margin-bottom:15px}.input-frm{float:left;width:100%;margin-bottom:15px}.input-frm .enable_section,.input-frm label{float:left;width:200px}.input-frm input{float:left;width:300px;border:1px solid #e1e1e1;padding:7px;border-radius:2px}.cont-form{padding:6px 15px}.ippopay-payment-section-form{float:left;width:100%}.input-frm input.update-set,.update-set{background-color:#001a6c;color:#fff;padding:10px 25px;border-radius:5px;float:left;width:300px;border-radius:5px;cursor:pointer;font-size:17px;font-weight:500;margin:10px 0 0 200px}.enable-check-label{float:left;width:100%;position:relative}.enable-check-label input{position:absolute;opacity:0;cursor:pointer;width:0;display:none}.enable-check-label .mark{top:0;position:absolute;height:24px;width:24px;background-color:transparent;border-radius:5px;transition:all .3s ease-in;border:2px solid #e1e1e1}.enable-check-label input:checked~.mark{background-color:#fff;border-radius:5px;transform:rotate(0) scale(1);opacity:1;border:2px solid #e1e1e1;cursor:pointer}.enable-check-label .mark::after{position:absolute;content:"";border-radius:5px}.enable-check-label input:checked~.mark::after{transform:rotate(45deg) scale(1);left:6px;top:2px;width:5px;height:10px;border:solid #001a6c;border-width:0 2px 2px 0;border-radius:0}';
        $this->html .= '<div class="ippopay-payment">
            <div class="ippopay-title">'.$this->displayName.'</div>
            <div class="ippopay-desc">
                <img src="../modules/ippopay/logo.png">
                <div class="ippopay-cont">
                    <h4 class="bld-cont">{$modDesc}</h4>
                    <p class="sub-desc1">{$modconfirm}</p>
                </div>
            </div>
        </div>';
        $this->_html .= '<h2>'.$this->displayName.'</h2>';
        $this->_html .= "<img src='../modules/ippopay/logo.png' style='float:left; margin-right:15px;' />
            <b>{$modDesc}</b>
            <br />
            <br />
            {$modStatus}
